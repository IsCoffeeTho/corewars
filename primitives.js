...
function dump(text)
{
	...
	{
		var word = ""
		for (var j = 0; j < (text.BYTES_PER_ELEMENT*8); j++)
		{
			word += (text[i] >>> (text.BYTES_PER_ELEMENT*8)-(j+1)) & 0b1;
			if ((j % 8) == 7) word += " ";
		}
		for (var j = 0; j < text.BYTES_PER_ELEMENT; j++)
			word += String.fromCharCode((text[i] >> ((text.BYTES_PER_ELEMENT*8)-((j+1)*8))) & 0xff);
		console.log(word);
	}
}

/** SHA 256 - Hashes any message you provide*/
function hash(text="")
{	
	/*
